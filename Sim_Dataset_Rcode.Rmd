...
d1[,4]<-ceiling(qweibull(pvars[,4],shape=0.5,scale=20))#Readmission
d1[,5]<-qpois(pvars[,3],lambda=5)# HbA1c (glycated haemoglobin)
d1[,6]<-round(qnorm(pvars[,6],mean=26,sd=3),0)#BMI
#Categorical variables
d1[,7] <-mycut(d1[,7], c(0, 1-prop_male, 1)) - 1#Gender
...
plot(d1[,"LoS"],d1[,"age"])
plot(d1[,"LoS"],d1[,"RU"])
plot(d1[,"LoS"],d1[,"readmission"])
plot(d1[,"RU"],d1[,"age"])

cor(d1[,"LoS"],d1[,"age"])
cor(d1[,"LoS"],d1[,"RU"])
cor(d1[,"LoS"],d1[,"readmission"])
cor(d1[,"RU"],d1[,"age"])

mod1<-lm(LoS~age,data=d1)
summary(mod1)
#plot(mod1)
mod2<-lm(LoS~RU,data=d1)
summary(mod2)

mod3<-lm(age~RU,data=d1)
summary(mod3)

mod4<-lm(readmission~LoS,data=d1)
summary(mod4)
